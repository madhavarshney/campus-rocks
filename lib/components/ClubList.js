...
function ClubCard({
  ...
  return (
    <div onClick={(e) => onClick()} className={styles.card} style={{ borderLeftColor: categories[category].color }}>
      {/* <h3>{name} [{category}]</h3> */}
      <div style={{ display: 'flex' }}>
        <h3 style={{ flex: 1, display: 'inline' }}>{name}</h3>{discord && (
        <a href={discord}>
          <img style={{ width: '28px', height: '28px' }} src="discord-icon-purple.svg"></img>
        </a>
      )}</div>
...
export default function ClubList({ categories, clubs, onClick }) {
  return (
    <div className={styles.grid}>
      {clubs.map(club => <ClubCard categories={categories} {...club} onClick={() => onClick(club)} />)}
    </div>
  )
}
