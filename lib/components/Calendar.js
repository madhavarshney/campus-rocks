...
export default function Calendar({ categories, clubs, onClubSelected }) {
  ...
  const timesInfo = useMemo(() => {
    ...
    for (const dayTimes of Object.values(times)) {
      dayTimes.sort(
        (a, b) =>
          parseInt(a.meetings.start_time.slice(0, 2), 10) -
          parseInt(b.meetings.start_time.slice(0, 2), 10)
      )
    }
  ...
  for (const day of days) {
    ...
    for (const club of timesInfo[day]) {
      ...
      const {
        name,
        category,
        meetings: { start_time, end_time, frequency },
      } = club
      ...
      timesDivs.push(
        ...
        >
          <h4>{name}</h4>
          <div>
            {parseTime(start_time)} - {parseTime(end_time)}
          </div>
          <div>{frequency}</div>
        </div>
      )
    ...
    if (timesInfo[day]) {
      mainDivs.push(
        <div key={day} className={styles.dayColumnContainer}>
          <div className={styles.dayTitle}>{day.slice(0, 3).toUpperCase()}</div>
          <div className={styles.dayColumn}>{timesDivs}</div>
        </div>
      )
    }
  }
  ...
  return (
    <div className={styles.calendarWrapper}>
      <div
        ...
        style={{ flexDirection: tabbedView ? 'column' : 'row' }}
      >
        {mainDivs}
      </div>
