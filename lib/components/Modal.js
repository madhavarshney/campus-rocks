    ...
    <div className={styles.card}>
      ...
      {meetings && (
        <p className={styles.repInfo}>
          <span className={styles.label}>Meetings:</span> {day}s {parseTime(start_time)} - {parseTime(end_time)}, {frequency}
        </p>
      )}
      {/* <p className={styles.repInfo}>Category: {categories[category].name}</p> */}
      {(email || club_representative) && (
        <p className={styles.repInfo}><span className={styles.label}>Email:</span> {email || club_representative.email}</p>
      )}
      {club_representative && (
        <p className={styles.repInfo}><span className={styles.label}>Club Representative:</span> {club_representative.name}</p>
      )}
      {advisor && (
        <p className={styles.repInfo}><span className={styles.label}>Faculty Advisor:</span> {advisor.name} {advisor.email || ''}</p>
      )}
    </div>
  )
}
